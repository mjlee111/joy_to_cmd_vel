...
JoyToCmdVelConverter::JoyToCmdVelConverter() : target_lin(0.0), target_ang(0.0), current_lin(0.0), current_ang(0.0)
{
  nh_ = ros::NodeHandle("~");
  ...
  rpm_ = nh_.advertise<joy_to_cmd_vel::JOY2RPM>("/joy2rpm", 10);
}

void JoyToCmdVelConverter::run()
{
  geometry_msgs::Twist cmd_vel_msg;
  joy_to_cmd_vel::JOY2RPM rpm;
  ...
  rpm.rpm_left = (current_lin - current_ang * WHEEL_RADIUS) * rpm_to_mps;
  rpm.rpm_right = (current_lin + current_ang * WHEEL_RADIUS) * rpm_to_mps;
